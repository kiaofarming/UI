...
const API_VER = "v1";
const mySwitch = document.querySelector('#bt1');
const ch_switch = [false, false]; // set initial switch status
  ...
  */
  ...
  window.onload = () => {
    mySwitch.addEventListener('change', () => {
      ch_switch[0] = mySwitch.checked;
      // Make API request to update switch status
      ...
      // Update switch UI
      if (mySwitch.checked) {
        mySwitch.parentNode.classList.add('on');
        document.getElementById("state").textContent = "ON";
      } else {
        mySwitch.parentNode.classList.remove('on');
